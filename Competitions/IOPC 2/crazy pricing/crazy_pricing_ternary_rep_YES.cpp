...
string ternary_representation(int n, int sz) {
    string ans = "";
    while(n>0) {
        int q = n/3;
        int r = n%3;
...
int main() {
    ...
    while(t--) {
        ...
        for (int mask=0 ; mask<pow(3, n) ; mask++) {
            reprs.push_back(ternary_representation(mask, n));
        }
        ...
        for (string repr:reprs) {
            ...
            for (int i=0 ; i<m ; i++) {
                ...
                if (
                    ...
                    && 
                    (repr[id1] != '1' or repr[id2] != '1') // one of them not bough
                ) {
                    tot = -1;
                    break;
                }
